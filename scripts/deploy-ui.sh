#!/usr/bin/env bash
# Build the projects plugin UI and rsync it to .57.
#
# Pairs with the nginx vhost change that points `project.4950.store` at
# `/Users/local/var/polar-ui/project/`. Cross-page sidebar links (e.g.
# /dashboard.html) fall back to dock's /ui/dist via nginx try_files, so
# this dir only needs to carry projects's own files.
#
# Required env:
...
#   DEPLOY_PATH   default: /Users/local/var/polar-ui/project
#
# Usage from the polar-projects repo root:
...
DEPLOY_PATH="${DEPLOY_PATH:-/Users/local/var/polar-ui/project}"
